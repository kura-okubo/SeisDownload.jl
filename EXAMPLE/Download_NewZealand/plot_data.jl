...
#----------------#
finame = "./dataset/NZnetwork.jld2"

plottimeunit = "min" #hour, min, sec

sr = 10 #downsampling for plotting

eq_normalize_amp = 2e-4 # amplitude normalization for earthquake
aftershock_α = 1.0 # amplitude normalization for aftershock (aftershock_α  * eq_normalize_amp)
starttimeid = 1 # start time id from timestamplist in SeisData
endtimeid = 25 # start time id from timestamplist in SeisData
stationID = 3 # choose station and components from stationlist in SeisData
...
for stid = starttimeid:endtimeid
	...
	if maximum(abs.(S.x)) > 18*std(S.x) && aftershockflag == 0
		# large earthquake occured -> 1 % of amp
		normalized_amp= eq_normalize_amp
		global aftershockflag = 1
	elseif maximum(abs.(S.x)) > 18*std(S.x) && aftershockflag == 1
		# aftershock
